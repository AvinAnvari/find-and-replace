...
function onHomepage() {
  ...
  builder.addSection(CardService.newCardSection()
    .addWidget(CardService.newTextParagraph().setText("Please specify the name you'd like on the folder which will contain the edited files:"))
    .addWidget(CardService.newTextInput()
      .setFieldName("newFolderName")
      .setTitle(`e.g. "Edited Files Folder"`)
      .setHint("Leave blank if you'd like the folder created within your own folder")
      .setValue("")
    )
    .addWidget(
      ...
      // )
        ...
        .setOnClickAction(
        CardService.newAction()
          .setFunctionName("openFolderPicker")
  )
)
    )
  );
...
function getPickerData() {
  return {
    ...
    appId: '894291576093',
    developerKey: 'AIzaSyB1hMl9N5T6e7NQOnMT8igN5vWQWqHlHEo'  // ← Your API key from Cloud Console
  };
}
...
function openFolderPicker(e) {
  const folderName = e?.commonEventObject?.formInputs?.newFolderName?.stringInputs?.value?.[0] ?? "";
  PropertiesService.getUserProperties().setProperty('newFolderName', folderName);
...
function getEachDocument(folderId) {
  const userResponse = PropertiesService.getUserProperties().getProperty('newFolderName') || "";
  ...
  if (userResponse === "") {
    newFolder = userFolder.createFolder(`${userFolder.getName()} Revised Copies`);
  } else {
  ...
  while (userFiles.hasNext()) {
    ...
    const newDoc = DriveApp.getFileById(template.getId()).makeCopy().moveTo(newFolder);
